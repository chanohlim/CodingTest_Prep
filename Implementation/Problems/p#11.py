...
for i in range(L):
    ...
    temp[0] = int(temp[0])
    change_d.append(temp)

change_time = change_d[0][0]
...
while alive:
    ...
    di, dj = head[0] + direction[d][0], head[1] + direction[d][1]
    ...
    if board[di][dj] != 2:
        prev_i, prev_j = Snake.popleft()
        board[prev_i][prev_j] = 0
    ...
    if change_time == time:
        current_direction = change_d.popleft()[1]
